...
try {
  ...
  if (!$user->_logged_in) {
   ...
   // get trending hashtags for guest welcome page
      $rowz = [];
      $get_rows = $db->query("SELECT hashtags.hashtag_id,hashtags.hashtag FROM hashtags LIMIT 5") or _error('SQL_ERROR');
      while ($row = $get_rows->fetch_assoc()) {
      $row['hashtag'] = get_tag($row['hashtag'], $row['hashtag']);
      $rowz[] = $row;
      }
      $smarty->assign('rowz', $rowz);
 
    // get latest users for guest welcome page
    $rows = [];
    $get_rows = $db->query("SELECT users.user_id,users.user_name,users.user_picture,users.user_activated FROM users WHERE users.user_activated = '1' LIMIT 8") or _error('SQL_ERROR');
    while ($row = $get_rows->fetch_assoc()) {
  ...
  $rowsu = [];
  $get_rows = $db->query("SELECT COUNT(user_id) FROM users") or _error('SQL_ERROR');
  while ($row = $get_rows->fetch_assoc()) {
  $rowsu[] = $row;
  }
  ...
  // Get total user count
  $rowsu = [];
  $get_rows = $db->query("SELECT COUNT(user_name) FROM users") or _error('SQL_ERROR');
  while ($row = $get_rows->fetch_assoc()) {
  $rowsu[] = $row;
  }
  $smarty->assign('rowsu', $rowsu);
